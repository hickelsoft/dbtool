...
uses
  ...
  Db, {$IFNDEF WIN64}DBTables, {$ENDIF}StdCtrls, ExtCtrls, Buttons, Gauges, ShellAPI;
...
type
  ...
  public
    ...
    property DateinameBasis: string read GetDateinameBasis write FDateinameBasis;
  end;
...
begin
   ...
   begin
     if (Components[iCounter] is TImage) and (TImage(Components[iCounter]).Picture.Bitmap.Empty) then
     begin
...
resourcestring
  SAreYouSureExportCancel = 'Sind Sie sicher, dass Sie den Datenexport abbrechen möchten?';
begin
   if FIsWorking then
     if Application.MessageBox(PChar(SAreYouSureExportCancel), PChar(Application.Title), MB_ICONQUESTION + MB_YESNOCANCEL) <> IDYES then exit;
   FIsWorking := false;
   Close;
end;
...
resourcestring
  SFieldTypeNotSupported = 'Achtung: Feld %s hat nicht unterstützten Feldtyp %d!';
begin
   ...
   begin
      ...
      begin
         ...
         begin
           ShowMessageFmt(SFieldTypeNotSupported, [FTable1.FieldDefs.Items[lCounter].Name, Integer(FTable1.FieldDefs.Items[lCounter].DataType)]);
           lbSrc.Items.Add(FTable1.FieldDefs.Items[lCounter].Name); // trotzdem probieren
         end;
      end;
   end
   else
   if Notebook1.ActivePage = '2' then
   begin
      ...
      begin
         ...
         case rgDateiformate.ItemIndex of
            RG_DATEIFORMAT_DBASE:   eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.dbf'; // do not localize
            RG_DATEIFORMAT_HTML:    eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.htm'; // do not localize
            RG_DATEIFORMAT_PARADOX: eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.db'; // do not localize
            RG_DATEIFORMAT_XML:     eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.xml'; // do not localize
            RG_DATEIFORMAT_SQL:     eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.sql'; // do not localize
         end;
      end;
   end
   else
   if Notebook1.ActivePage = '3' then
   begin
      Notebook1.ActivePage := '5';
      btnWeiter.Enabled := false;
      btnFertig.Enabled := true;
      eDateiname.Text := IncludeTrailingPathDelimiter(DefaultExportDir) + DateinameBasis + '.csv'; // do not localize
   end;
...
begin
   ...
   if Notebook1.ActivePage = '5' then
   begin
      case rgDateiformate.ItemIndex of
         ...
         RG_DATEIFORMAT_SQL,
         RG_DATEIFORMAT_XML:  Notebook1.ActivePage := '2';
         RG_DATEIFORMAT_CSV:  Notebook1.ActivePage := '3';
      end;
      btnWeiter.Enabled := true;
      btnFertig.Enabled := false;
   end;
...
procedure TDLG_Export.lbDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
...
procedure TDLG_Export.lbMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
   IncludeBtn.Enabled := (lbSrc.SelCount > 0);
   ExcludeBtn.Enabled := (lbDst.SelCount > 0);
end;
...
begin
   for iCounter := 0 to lbSrc.Items.Count-1 do lbSrc.Selected[iCounter] := true;
   lbDragDrop(lbDst, lbSrc, 0, 0);
end;
...
begin
   for iCounter := 0 to lbDst.Items.Count-1 do lbDst.Selected[iCounter] := true;
   lbDragDrop(lbSrc, lbDst, 0, 0);
end;
...
var
  iBuffer: integer;
resourcestring
  SOnlyMoveOneElement = 'Es kann immer nur ein einzelnes Element auf einmal bewegt werden!';
begin
   ...
   begin
     Application.MessageBox(PChar(SOnlyMoveOneElement), PChar(Application.Title), MB_ICONEXCLAMATION + MB_OK)
   end
   else if lbDst.SelCount = 1 then
   begin
      iBuffer := lbDst.ItemIndex - (Sender as TSpeedButton).Tag;
      if iBuffer < 0 then exit else if iBuffer > lbDst.Items.Count-1 then exit;
...
begin
   ...
   labelText := StringReplace(labelText, '<BR>', #13#10, [rfReplaceAll, rfIgnoreCase]); // do not localize
...
begin
   ...
   begin
      case rgDateiformate.ItemIndex of
         RG_DATEIFORMAT_DBASE:   eDateiname.Text := changefileext(SaveDialog1.FileName, '.dbf'); // do not localize
         RG_DATEIFORMAT_HTML:    eDateiname.Text := changefileext(SaveDialog1.FileName, '.htm'); // do not localize
         RG_DATEIFORMAT_PARADOX: eDateiname.Text := changefileext(SaveDialog1.FileName, '.db'); // do not localize
         RG_DATEIFORMAT_CSV:     eDateiname.Text := changefileext(SaveDialog1.FileName, '.csv'); // do not localize
         RG_DATEIFORMAT_XML:     eDateiname.Text := changefileext(SaveDialog1.FileName, '.xml'); // do not localize
         RG_DATEIFORMAT_SQL:     eDateiname.Text := changefileext(SaveDialog1.FileName, '.sql'); // do not localize
      end;
   end;
end;
...
var
  ...
  {$ENDIF}
   ...
   begin
      sText := StringReplace(sText, 'Ä', '&Auml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'Ö', '&Ouml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'Ü', '&Uuml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'ä', '&auml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'ö', '&ouml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'ü', '&uuml;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, 'ß', '&szlig;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, '<', '&lt;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, '>', '&gt;', [rfReplaceAll]); // do not localize
      sText := StringReplace(sText, #13, '<br>'+#13, [rfReplaceAll]); // do not localize
      Result := sText;
   end;
...
begin
   ...
   begin
     if MessageDlg(Format(SFileAlreadyExistsOverwrite, [eDateiname.Text]), mtWarning, mbYesNoCancel, 0) <> mrYes then Abort;
   end;
   ...
   btnZurueck.Enabled := False;
   case rgDateiformate.ItemIndex of
      RG_DATEIFORMAT_DBASE:   lExportFormat.Caption := SExportDBaseRunning;
      RG_DATEIFORMAT_HTML:    lExportFormat.Caption := SExportHtmlRunning;
      RG_DATEIFORMAT_PARADOX: lExportFormat.Caption := SExportParadoxRunning;
      RG_DATEIFORMAT_CSV:     lExportFormat.Caption := SExportCsvRunning;
      RG_DATEIFORMAT_XML:     lExportFormat.Caption := SExportXmlRunning;
      RG_DATEIFORMAT_SQL:     lExportFormat.Caption := SExportSqlDumpRunning;
   end;

   if rgDateiformate.ItemIndex in [RG_DATEIFORMAT_DBASE,RG_DATEIFORMAT_PARADOX] then  // dBase- oder Paradox-Export ----
   begin
      {$IFNDEF WIN64}
      ...
      try
        ...
        try
          ...
          begin
             ...
             for iCounter := 0 to ttNeu.Fields.Count-1 do ttNeu.Fields[iCounter].Value := FTable1.FieldByName(ttNeu.Fields[iCounter].FieldName).Value;
   ...
   else if rgDateiformate.ItemIndex = RG_DATEIFORMAT_SQL then  // SQL Dump ------------------------
   begin
      ...
      try
        ...
        begin
           ...
           begin
             ...
             sFeldInhalt := FieldStr(FTable1.FieldByName(lbDst.Items[iCounter]), false);
             if not FTable1.FieldDefs.Items[iCounter].Required and (sFeldInhalt = '') then
             begin
           ...
           // TODO: escape
           sZeilenBuffer := 'insert into ' + sTableName + ' ('+felder+') values ('+values+');'; // do not localize
   ...
   else if rgDateiformate.ItemIndex = RG_DATEIFORMAT_HTML then  // HTML-Export ------------------------
   begin
      // Erst mal die neue Datei anlegen
      AssignFile(F, eDateiname.Text);
      Rewrite(F);
      writeln(F, '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">'); // do not localize
      writeln(F, '<!-- '+SDBToolHtmlComment+' -->'); // do not localize
      writeln(F, '');
      writeln(F, '<HTML><HEAD><TITLE>' + ToHtml(sTableName) + '</TITLE></HEAD>'); // do not localize
      writeln(F, '<BODY bgColor="#ffffff"><CENTER><H1>' + ToHtml(sTableName) + '</H1><BR><TABLE BORDER="1">'); // do not localize

      // Feldnamen in die erste Zeile
      sZeilenBuffer := '<TR>'; // do not localize
      for iCounter := 0 to lbDst.Items.Count-1 do
      begin
        sZeilenBuffer := sZeilenBuffer + '<TD>' + ToHtml(lbDst.Items[iCounter]) + '</TD>'; // do not localize
      end;
      ...
      try
        ...
        begin
           ...
           begin
              // TODO: escape
              sZeilenBuffer := sZeilenBuffer + '<TD>' + ToHtml(FieldStr(FTable1.FieldByName(lbDst.Items[iCounter]), true)) + '</TD>'; // do not localize
           end;
   ...
   else if rgDateiformate.ItemIndex = RG_DATEIFORMAT_CSV then   // Text-Export -----------------------
   begin
      // Erst mal die neue Datei anlegen
      AssignFile(F, eDateiname.Text);
      Rewrite(F);
      ...
      case rgTrennzeichen.ItemIndex of
         ...
         3: sTrennzeichen := #32;
         4: sTrennzeichen := '/';
         5: sTrennzeichen := eTrennzeichen.Text;
      end;
      ...
      begin
         ...
         for iCounter := 1 to lbDst.Items.Count-1 do sZeilenBuffer := sZeilenBuffer + sTrennzeichen + lbDst.Items[iCounter];
         writeln(F, sZeilenBuffer);
      end;
      ...
      try
        ...
        begin
           ...
           begin
              // TODO: escape!
              sBuffer := StringReplace(FieldStr(FTable1.FieldByName(lbDst.Items[iCounter]), false), sTexterkennung, sTexterkennung + sTexterkennung, [rfReplaceAll, rfIgnoreCase]);
              if (pos(sTrennzeichen, sBuffer) > 0) or (pos(sTexterkennung, sBuffer) > 0) then sBuffer := sTexterkennung + sBuffer + sTexterkennung;
           ...
           sZeilenBuffer := copy(sZeilenBuffer, 1, length(sZeilenBuffer)-length(sTrennzeichen));
   ...
   else if rgDateiformate.ItemIndex = RG_DATEIFORMAT_XML then  // XML-Export ------------------------
   begin
      // Erst mal die neue Datei anlegen
      AssignFile(F, eDateiname.Text);
      Rewrite(F);
      writeln(F, '<?xml version="1.0" encoding="ISO-8859-1"?>'); // do not localize
      writeln(F, '<!-- '+SDBToolHtmlComment+' -->'); // do not localize
      writeln(F, '');
      ...
      try
        ...
        begin
           ...
           begin
             // TODO: Escape
             writeln(F, '    <' + lbDst.Items[iCounter] + '>' + FieldStr(FTable1.FieldByName(lbDst.Items[iCounter]), false) + '</' + lbDst.Items[iCounter] + '>');
           end;
           writeln(F, '  </RECORD>'); // do not localize
   ...
   FIsWorking := false;
   Application.MessageBox(PChar(SExportDone), PChar(Application.Title), MB_ICONINFORMATION + MB_OK);
   if not (rgDateiformate.ItemIndex in [RG_DATEIFORMAT_DBASE,RG_DATEIFORMAT_PARADOX]) then
   begin
     ShellExecute64(Handle, nil, 'explorer.exe', PChar('/select,'+eDateiname.Text), nil, SW_SHOWNORMAL); // do not localize
   end;
   Close;
end;
