...
uses
  Windows, SysUtils, Classes, Controls, StdCtrls, Forms, ExtCtrls, ComCtrls,
  Graphics,
...
type
  ...
  public
    ...
    constructor Create(Owner: TComponent; aDB: TDbToolDatabase; aDS: TDataSet;
      sTable: String); reintroduce;
  end;
...
constructor TDLG_Import.Create(Owner: TComponent; aDB: TDbToolDatabase;
  aDS: TDataSet; sTable: String);
begin
...
procedure TDLG_Import.FormKeyDown(Sender: TObject; var Key: word;
  Shift: TShiftState);
begin
...
begin
  ...
  if (FCSVLines.Count < 1) then
    raise Exception.Create(SSourceFileEmpty);
  ...
  if (rbTabStop.Checked) then
    ...
    raise Exception.Create(SDelimitermissing);
  ...
  try
    ...
    iMaxLines := 50 + iLine;
    if (FCSVLines.Count < 50) then
      iMaxLines := FCSVLines.Count;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          lbSrc.ItemIndex := 0;
          // muss 0 sein, denn bei btnAddClick werden die Felder nach oben verschoben
          lbDst.ItemIndex := 0;
          btnAddClick(Sender);
        end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          if SameText(Trim(lbDst.Items.Strings[i]), Trim(lbSrc.Items.Strings[j]))
          then
          begin
            lbSrc.ItemIndex := j;
            lbDst.ItemIndex := i;
...
resourcestring
  SAreYouSureCancelImport =
    'Die Daten wurden noch nicht vollständig eingelesen. Sind Sie sicher, dass Sie den Import abbrechen wollen?';
begin
  if (FRunning) then
  begin
    if Application.MessageBox(PChar(SAreYouSureCancelImport),
...
begin
  ...
  lbVerknuepfungen.Items.Add(Trim(lbSrc.Items.Strings[lbSrc.ItemIndex]) + #9 +
    '.' + #9 + Trim(lbDst.Items.Strings[lbDst.ItemIndex]));
...
begin
  if (lbSrc.ItemIndex <> -1) and (lbDst.ItemIndex <> -1) then
    btnAdd.Enabled := true;
end;
...
begin
  if (lbVerknuepfungen.ItemIndex <> -1) then
...
resourcestring
  ...
  SError = 'Fehler';
  SSQL = 'SQL';
  SImportFinishedDDatasetsDErrors =
    'Import abgeschlossen (%d Datensätze erzeugt, %d Fehler)';
...
begin
  ...
  // Vorbereitungen...
  if lbVerknuepfungen.Items.Count = 0 then
    raise Exception.Create(SNoFieldsAssociated);
  btnZurueck.Enabled := false;
  btnFertig.Enabled := false;
  ...
  Memo2.Lines.Clear;
  Memo2.Lines.Add(FormatDateTime('dd.mm.yyyy hh:nn:ss', Now) + ' ' +
    SImportStarted); // do not localize
  // Memo2.Lines.Add('');
  cText := eTexterkennung.Text[1];
  if (rbTabStop.Checked) then
    cSep := #9
  ...
  try
    ...
    Csv2Sl(FCSVLines.Strings[0], slBuffer, cSep, cText);
    // In slBuffer stehen jetzt die Feldnamen der Quelldatei in der Original-Reihenfolge
    for i := 0 to slBuffer.Count - 1 do
    begin
      if cbFeldnamenInErsterZeile.Checked then
    ...
    begin
      ...
      saFelder[slBuffer.IndexOf(Copy(sBuffer, 1, Pos(#9, sBuffer) - 1))] :=
        Copy(sBuffer, Pos(#9, sBuffer) + 3);
    end;

    // Insert-Anweisung vorbereiten
    sInsertClause := 'INSERT INTO ' + FDatabase.SQL_Escape_TableName(FTableName)
      + '('; // do not localize
    for i := 0 to slBuffer.Count - 1 do
    begin
      if saFelder[i] <> '' then
        sInsertClause := sInsertClause + FDatabase.SQL_Escape_FieldName
          (saFelder[i]) + ',';
    end;
    sInsertClause := Copy(sInsertClause, 1, Length(sInsertClause) - 1) +
    ...
    begin
      ...
      begin
        Memo2.Lines.Add(FormatDateTime('dd.mm.yyyy hh:nn:ss', Now) + ' ' +
          SEmptyLineSkipped); // do not localize
      end
      else
      begin
        Csv2Sl(sLine, slBuffer, cSep, cText);
        ...
        begin
          ...
          begin
            ...
            if aFieldType in [ftSmallint, ftInteger, ftWord, ftAutoInc,
              ftLargeint] then
            begin
            ...
            begin
              if slBuffer.Strings[i] = '' then
                ...
                sBuffer := sBuffer + '0,'
              else if SameText(slBuffer.Strings[i], 'wahr') then
              // do not localize
                sBuffer := sBuffer + '1,'
              else if SameText(slBuffer.Strings[i], 'falsch') then
              // do not localize
                sBuffer := sBuffer + '0,'
              else if SameText(slBuffer.Strings[i], 'true') then
              // do not localize
                sBuffer := sBuffer + '1,'
              else if SameText(slBuffer.Strings[i], 'false') then
              // do not localize
                sBuffer := sBuffer + '0,'
              else if SameText(slBuffer.Strings[i], 'yes') then
              // do not localize
                sBuffer := sBuffer + '1,'
              else if SameText(slBuffer.Strings[i], 'no') then
              // do not localize
                sBuffer := sBuffer + '0,'
              else if SameText(slBuffer.Strings[i], 'ja') then
              // do not localize
                sBuffer := sBuffer + '1,'
              else if SameText(slBuffer.Strings[i], 'nein') then
              // do not localize
                sBuffer := sBuffer + '0,'
              else
                sBuffer := sBuffer + 'null,';
              // TODO: sollte nicht passieren! // do not localize
            end
            ...
            else if aFieldType in [ftString, ftMemo, ftFmtMemo, ftFixedChar,
              ftWideString, ftFixedWideChar, ftWideMemo] then
            begin
              sBuffer := sBuffer + '''' + StringReplace(slBuffer.Strings[i],
                '''', '''''', [rfReplaceAll]) + ''',';
            end
        ...
        try
          ...
          begin
            Inc(cntErr);
            Memo2.Lines.Add(FormatDateTime('dd.mm.yyyy hh:nn:ss', Now) + ' ' +
              SError + ': "' + E.Message + '", ' + SSQL + ': "' + Copy(sBuffer,
              1, Length(sBuffer) - 1) + ');"'); // do not localize
          end;
        end;
  ...
  // Memo2.Lines.Add('');
  Memo2.Lines.Add(FormatDateTime('dd.mm.yyyy hh:nn:ss', Now) + ' ' +
    format(SImportFinishedDDatasetsDErrors, [cntDS, cntErr]));
  // do not localize
  ...
  if not FRunning then
    Label7.Caption := SImportFinished;

  btnZurueck.Enabled := true;
  // Man will bei einem Fehler ggf. nochmal probieren.
end;
...
begin
  if Assigned(FCSVLines) then
    FreeAndNil(FCSVLines);
end;
