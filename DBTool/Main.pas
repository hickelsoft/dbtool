...
begin
  ...
  result := '';
  iniClient := TMemIniFile.Create
    (IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0))) + CONFIG_INI);
  try
    sMode := iniClient.ReadString(CONFIG_HEADER, 'Mode', ''); // do not localize
    if sMode = 'Client' then // do not localize
    begin
      sSQLServerPath := iniClient.ReadString(CONFIG_HEADER, 'SQLConnStr', '');
      // do not localize
      ...
      begin
        sServerPath := iniClient.ReadString(CONFIG_HEADER, 'ServerPath', '');
        // do not localize
        ...
        try
          if iniServer.ReadString(CONFIG_HEADER, 'Mode', '') <> 'Server' then
          // do not localize
          begin
            raise Exception.CreateFmt(SModeIsNotS, [sServerIni, 'Server']);
            // do not localize
          end;
  ...
  result := StringReplace(result, 'Application Name', 'emaN noitacilppA',
  ...
  result := StringReplace(result, '???', HS_SA_DB_PASSWORD, []);
  // do not localize

  // Automatisch den besten SQL Client Treiber wählen
  result := StringReplace(result, 'Provider=(Auto)',
...
begin
  ...
  begin
    ...
    // Wenn nicht, Datenbank neu öffnen!
    if not Assigned(aDbFrm) then { aDbFrm := }
      TMDI_Database.Create(Self, FileName);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      OpenFile('_SQLSRV:Initial Catalog=' + aDlg.lbDatabases.Items.Strings
        [aDlg.lbDatabases.ItemIndex] + ';Data Source=' + aDlg.eServer.Text +
        ';'); // do not localize
    end;
  finally
    FreeAndNil(aDlg);
...
begin
  Application.MessageBox(PChar(SMySqlUnstable), PChar(Application.Title),
  ...
  try
    ...
    begin
      ...
      OpenFile('_MYSQL:database=' + aDlg.lbDatabases.Items.Strings
        [aDlg.lbDatabases.ItemIndex] + ';server=' + aDlg.eServer.Text + ';');
      // do not localize
    end;
  finally
    FreeAndNil(aDlg);
...
begin
  ...
  try
    clActiveField := TColor(GetNearestColor(Canvas.Handle,
      ColorToRGB(StringToColor(aReg.ReadString('Colors', 'ActiveField',
      '$0080FFFF'))))); // do not localize
    clActiveRecord := TColor(GetNearestColor(Canvas.Handle,
      ColorToRGB(StringToColor(aReg.ReadString('Colors', 'ActiveRecord',
      '$00DED3D6'))))); // do not localize
    clTableBackground := TColor(GetNearestColor(Canvas.Handle,
      ColorToRGB(StringToColor(aReg.ReadString('Colors', 'Background',
      'clWhite'))))); // do not localize
    clTableText := TColor(GetNearestColor(Canvas.Handle,
      ColorToRGB(StringToColor(aReg.ReadString('Colors', 'Text', 'clBlack')))));
    // do not localize
    clTableZebra := TColor(GetNearestColor(Canvas.Handle,
      ColorToRGB(StringToColor(aReg.ReadString('Colors', 'Zebra', '$00EBFFFF')))
      )); // do not localize
  finally
    FreeAndNil(aReg);
  end;
...
begin
  ...
  if (Copy(Msg.Strings[0], 1, 7) = '[open("') and not Modus_CORA_Verzeichnis and
    not Modus_HsInfo2_Verzeichnis then // do not localize
  begin
...
resourcestring
  SLocalMd5Error =
    'DBTool kann nicht nach Updates suchen, da es nicht fest auf diesen Computer installiert ist. Bei portablen Versionen schauen Sie bitte regelmäßig auf github.com/hickelsoft/dbtool nach Updates.';
  SRemoteMd5Error =
    'Fehler beim Abfragen der Prüfsumme vom Server. Bitte prüfen Sie Ihre Internetverbindung oder schauen Sie auf github.com/hickelsoft/dbtool nach Updates';
...
begin
  if Modus_CORA_Verzeichnis or Modus_HsInfo2_Verzeichnis then
    exit;
  iniFileName := IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0))) +
    'Setup.ini'; // do not localize
  ...
  try
    localSetupMd5 := ini.ReadString('Setup', 'InstalledMD5', '');
    // do not localize
    if Length(Trim(localSetupMd5)) <> 32 then
      raise Exception.Create(SLocalMd5Error);

    try
      remoteSetupMd5 := DoGet('https://www.hickelsoft.de/dbtool/checksum.php');
      // do not localize
    except
      on E: EAbort do
      begin
    ...
    begin
      if Application.MessageBox(PChar(SNewVersionAvailable),
        PChar(Application.Title), MB_ICONINFORMATION + MB_YESNOCANCEL) = ID_YES
      then
      begin
    ...
    begin
      Application.MessageBox(PChar(SNoNewVersion), PChar(Application.Title),
        MB_ICONINFORMATION + MB_OK);
    end;
  finally
    FreeAndNil(ini);
...
var
  aDlg: TDLG_Import;
begin
  if not(ActiveMDIChild is TMDI_Table) then
    exit;
  aDlg := TDLG_Import.Create(Self, TMDI_Table(ActiveMDIChild)
    .frmDatabase.dbDatabase, TMDI_Table(ActiveMDIChild).dsData.DataSet,
    TMDI_Table(ActiveMDIChild).Table);
  try
    aDlg.ShowModal;
...
begin
  ...
  begin
    ...
    begin
      anItem.Caption := TMDI_Query(MDIChildren[i]).Database.DatabaseName;
      // TODO: Da haben alle den gleichen Titel
...
begin
  for i := 0 to MDIChildCount - 1 do
  begin
    if (MDIChildren[i].Tag <> 0) and (MDIChildren[i].Tag = TMenuItem(Sender).Tag)
    then
    begin
...
begin
  for i := 0 to MDIChildCount - 1 do
  begin
    if MDIChildren[i].ClassNameIs('TMDI_Query') then
      MDIChildren[i].Close; // do not localize
  end;
end;
...
begin
  for i := 0 to MDIChildCount - 1 do
  begin
    if MDIChildren[i].ClassNameIs('TMDI_Table') then
      MDIChildren[i].Close; // do not localize
  end;
end;
...
resourcestring
  ...
  SBackgroundText2 = '';
begin
  THickelSOFTDesign.ZeichneHintergrundschrift(SBackgroundText1,
    SBackgroundText2, 0.6);
end;
...
begin
  ...
  else if (ParamCount > 0) and (ParamStr(1) <> '-Embedding') then
  // do not localize
  begin
