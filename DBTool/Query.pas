...
begin
  ...
  QueryStatusPanel.Caption := '  ' + Format(SDatasetsD, [TDataSource(Sender).DataSet.RecordCount/1.0]);
end;
...
begin
  ...
  begin
    MDI_Form_BringToFront(Self); // TODO: - funktioniert bei 2 fenstern und beim dritten nicht?! (Fehler nur in Delphi 11)
    ...
    CanClose := Application.MessageBox(PChar(SReallyClose), PChar(Application.Title), MB_ICONQUESTION + MB_YESNOCANCEL) = ID_YES;
  end
  else
    CanClose := true;
end;
...
procedure TMDI_Query.dbgTableCalcCellColors(Sender: TObject; Field: TField; State: TGridDrawState; Highlight: boolean; AFont: TFont; ABrush: TBrush);
begin
...
procedure TMDI_Query.dbgTableKeyUp(Sender: TObject; var Key: word; Shift: TShiftState);
begin
...
begin
  if not FShareDBObj then FreeAndNil(FDatabase);
  inherited;
end;
...
begin
  if not dbgTable.Visible then exit;
...
resourcestring
  STableEndReached = 'Tabellen-Ende erreicht. Es wurden keine weiteren Vorkommen des Suchtexts gefunden.';
begin
  result := true;
  Inc(iSearchField);

  while (iSearchField < DataSource1.DataSet.FieldCount) and (DataSource1.DataSet.Fields.Fields[iSearchField].Visible = false) do
  begin
    Inc(iSearchField);
  end;
  ...
  begin
    ...
    begin
      ...
      while (iSearchField > 0) and not DataSource1.DataSet.Fields.Fields[iSearchField].Visible do
      begin
        Dec(iSearchField);
      end;
      dbgTable.SetActiveField(DataSource1.DataSet.Fields.Fields[iSearchField].FieldName);
      Application.MessageBox(PChar(STableEndReached), PChar(Application.Title), MB_ICONINFORMATION + MB_OK);
      result := false;
    end;
  end;
end;
...
begin
  ...
  if f = nil then exit; // can happen if you had a syntax error before
  ...
  begin
    try
      if(FFindCaseInsensitive) then
        iPos := Pos(sUpper, UpperCase(DataSource1.DataSet.Fields.Fields[iSearchField].AsString))
      else
...
begin
  ...
  if Value then Memo1.Color := clBtnFace else Memo1.Color := clWindow;
...
begin
  ...
  sGesamt := Trim(StringReplace(StringReplace(sGesamt, #13, ' ', [rfReplaceAll]), #10, ' ', [rfReplaceAll]));

  if sGesamt = '' then exit;

  bLastWasQuery := false;

  if (sGesamt <> '') and (
    (ContainsText(sGesamt, ' View')) or // do not localize
    (ContainsText(sGesamt, ' Procedure') and not ContainsText(sGesamt, '-- '+SExecuteStoredProcedureWith_+' ')) or // do not localize
    (ContainsText(sGesamt, ' Trigger') and not ContainsText(sGesamt, '-- '+STriggerActived) and not ContainsText(sGesamt, '-- '+STriggerDeActived))) then // do not localize
  begin
    if Assigned(DataSource1.DataSet) then
    begin
      DataSource1.DataSet.Free;
  ...
  dbgTable.Visible := true; // war bug: wenn man im query fenster ist und auf den leeren dbgrid klickt und dann eine taste drückt, kommt listenindex fehler!

  if not bLastWasQuery then
    Application.MessageBox(PChar(SSqlExecuted), PChar(Application.Title), MB_ICONINFORMATION + MB_OK);
end;
