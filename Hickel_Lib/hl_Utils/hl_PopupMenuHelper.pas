...
type
  TPopupMenuHelper = class helper for TPopupMenu
    procedure OpenPopupOnControl(ctl: TControl; OnlyIfAutopopup: boolean=True);
  end;

implementation

procedure TPopupMenuHelper.OpenPopupOnControl(ctl: TControl; OnlyIfAutopopup: boolean=True);
var
  lPoint: TPoint;
  oPoint: TPoint;
begin
  if OnlyIfAutopopup and not Self.AutoPopup then exit;
...
//     p.x-1, p.y, 0, handle, nil);
  ...
  *)
  ...
  if (Mouse.CursorPos.X < oPoint.X) or
     (Mouse.CursorPos.X > oPoint.X + ctl.Width) or
     (Mouse.CursorPos.Y < oPoint.Y) or
     (Mouse.CursorPos.Y > oPoint.Y + ctl.Height) then
  begin
