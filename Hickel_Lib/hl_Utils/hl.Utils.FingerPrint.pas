...
type
  ...
  strict protected
    ...
    FLogFile: string;
  public
    function Verify(Certificate: TIdX509; AOk: Boolean{$IFDEF NewIndy}; ADepth, AError: Integer{$ENDIF}): Boolean; virtual; abstract;
    constructor Create(AExpected: string; ALogFile: string='');
  end;

  TCertFingerPrintVerify = class(TCertFingerPrintVerifyBase)
  public
    function Verify(Certificate: TIdX509; AOk: Boolean{$IFDEF NewIndy}; ADepth, AError: Integer{$ENDIF}): Boolean; override;
  end;
...
constructor TCertFingerPrintVerifyBase.Create(AExpected: string; ALogFile: string='');
begin
  inherited Create;
...
function TCertFingerPrintVerify.Verify(Certificate: TIdX509; AOk: Boolean{$IFDEF NewIndy}; ADepth, AError: Integer{$ENDIF}): Boolean;
var
  sl: TStringList;
begin
  {$IFDEF NewIndy}
  result := (FExpected = '') or
            ...
            SameText('sha1/'+Certificate.Fingerprints.SHA1AsString, Trim(FExpected)) or
            SameText('sha224/'+Certificate.Fingerprints.SHA224AsString, Trim(FExpected)) or
            SameText('sha256/'+Certificate.Fingerprints.SHA256AsString, Trim(FExpected)) or
            SameText('sha384/'+Certificate.Fingerprints.SHA384AsString, Trim(FExpected)) or
            SameText('sha512/'+Certificate.Fingerprints.SHA512AsString, Trim(FExpected));
