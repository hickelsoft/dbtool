...
type
  ...
  private
    ...
    function Passt(dataRow: TDataSet; suchbegriffListe: TStringList)
      : boolean; overload;
  public
...
begin
  ...
  for i := 0 to woerter.Count - 1 do
    if (pos(suche, woerter[i]) > 0) then
      result := true;

end;

function ThlSuche.Passt(dataRow: TDataSet;
  suchbegriffListe: TStringList): boolean;
var
  i: integer;
  buffer: string;
...
begin
  ...
  for i := 0 to wortListe.Count - 1 do
    woerter2.Add(wortListe[i]);

  // Wenn alle der Suchbegriffe in der wortListe enthalten ist, Result := true und raus!
  result := true;
  for i := 0 to suchbegriffListe.Count - 1 do
    result := (result) and
      (((Passt(suchbegriffListe[i], wortListe)) or (Passt(suchbegriffListe[i],
      woerter2))));
end;
...
begin
  ...
  try
    ...
    suchbegriffe := StringReplace(AnsiLowerCase(suchbegriffe) + ' ', '  ', ' ',
      [rfReplaceAll]);
      ...
      begin
        ...
        begin
          ...
          for i := 0 to daten.FieldCount - 1 do
            ret.Fields[i].Value := daten.Fields[i].Value;
          ret.Post;
        end;
        daten.Next;
      end;
