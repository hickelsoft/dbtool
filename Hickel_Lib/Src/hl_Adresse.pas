...
begin
   result := mName1;
   if (mName2 <> '') and (mName2 <> '.') then result := result + #13#10 + mName2;
   if (mStrasse <> '') and (mStrasse <> '.') then result := result + #13#10 + mStrasse;
   if (mOrt <> '') and (mOrt <> '.') then result := result + #13#10 + mOrt;
end;

function ThlAdresse.GetLand: string;
begin
  if (mLand = '') or (mLand = ' ') then result := 'D' else result := mLand;
