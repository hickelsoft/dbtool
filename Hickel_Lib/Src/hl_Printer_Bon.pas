...
type
  ThlPrinterBon = class(ThlPrinterNeu)
    procedure BondruckerControlCode(aCommand: Char; aPrinterName: string; aSchachtIndex: integer);
  end;
...
threadvar
  _hlPrinterBon: ThlPrinterBon;

procedure ThlPrinterBon.BondruckerControlCode(aCommand: Char; aPrinterName: string; aSchachtIndex: integer);
var
  iNoFeedNoCutSchachtIndex: integer;
begin
  if aPrinterName = '' then exit;
  if aPrinterName = 'Bildschirm' then exit; // Bildschirmdruck. Nichts machen
  ...
  if ContainsText(PrinterName, 'Samsung') or ContainsText(PrinterName, 'Bixolon') then
    Canvas.Font.Name := 'FontControl' { Control Function Font Name für Bixolon / Samsung SRP 350 }
  else if ContainsText(PrinterName, 'Epson') then
    Canvas.Font.Name := 'control'     { Control Function Font Name für Epson TM-T88 }
  else
...
function hlPrinterBon: ThlPrinterBon;
begin
  if (_hlPrinterBon = nil) then _hlPrinterBon := ThlPrinterBon.Create;
