...
{$IFEND}
  ...
  published
    ...
    Property PrinterDataKopien: TPrinterData read FPrinterDataKopien
      write FPrinterDataKopien;
  end;
...
function THsDruckerConfig.WritePrinter: boolean;
  ...
  var
    ds: TADODataSet;
  begin
    if PrinterData = nil then
      exit;
    if FFormularArt = '' then
      exit;

    FConnection.BeginTrans;
    try
      FConnection.Execute
        ('delete from DRUCKFORMEINST_POS where FORMULARART = ''' + FFormularArt
        + ''' and FORMULARNR = ' + IntToStr(FFormularNr) +
        ' and DRUCKERART = ''' + FDruckerArt + ''' and LFD_DRUCKNR = ' +
        IntToStr(DruckerNr) + ' and BAP = ''' + GetComputerName + ''';');
      // TODO: escaping
      ...
      try
        ...
        if PrinterData.Zufuhrliste <> nil then
          ds.FieldByName('ZUFUHRINDEX').AsInteger :=
            PrinterData.Zufuhrliste.Itemindex
        else
          ds.FieldByName('ZUFUHRINDEX').AsInteger := -1;

        if PrinterData.DuplexListe <> nil then
          ds.FieldByName('DUPLEXINDEX').AsInteger :=
            PrinterData.DuplexListe.Itemindex
        else
          ds.FieldByName('DUPLEXINDEX').AsInteger := -1;

        if PrinterData.SpinKopien <> nil then
          ds.FieldByName('ANZAHLAUSDRUCKE').AsInteger :=
            PrinterData.SpinKopien.Value
        else
          ds.FieldByName('ANZAHLAUSDRUCKE').AsInteger := -1;

        ds.FieldByName('DRUCKEN_KOPF').AsBoolean := PrinterData.DruckenKopf;
        ds.FieldByName('DRUCKEN_ADRESSÜBERSCHRIFT').AsBoolean :=
          PrinterData.DruckenAdressueberschrift;
        ...
        ds.FieldByName('KOPFSEITE2ANDERS').AsBoolean :=
          PrinterData.Kopfseite2Anders;
...
function THsDruckerConfig.ReadPrinter: boolean;
  ...
  begin
    ...
    try
      ...
      ds.ParamCheck := false;
      ds.CommandText :=
        'select * from DRUCKFORMEINST_POS where FORMULARART = ''' + FFormularArt
        + ''' and FORMULARNR = ' + IntToStr(FFormularNr) +
        ' and DRUCKERART = ''' + FDruckerArt + ''' and LFD_DRUCKNR = ' +
        IntToStr(DruckerNr) + ' and BAP = ''' + GetComputerName + ''';';
      // TODO: escaping (hl.System.Types gibt es hier noch nicht)
      ds.Active := true;
      ds.DisableControls;
      ...
      begin
        ...
        begin
          ...
          if PrinterData.Zufuhrliste <> nil then
            PrinterData.Zufuhrliste.Itemindex := iBuffer;
          ...
          if PrinterData.DuplexListe <> nil then
            PrinterData.DuplexListe.Itemindex := iBuffer;
          if PrinterData.SpinKopien <> nil then
            PrinterData.SpinKopien.Value := ds.FieldByName('ANZAHLAUSDRUCKE')
              .AsInteger;
        end;
        ...
        PrinterData.Kopfseite2Anders := ds.FieldByName('KOPFSEITE2ANDERS')
          .AsBoolean;
