...
{$IFEND}
  ...
  published
    ...
    property AutoGetInfo: Boolean read FAutoGetInfo write SetAutoGetInfo
      default True;
...
begin
  ...
  try
    ...
    FHaveVersionInfo := GetFileVersionInfo(FFileName, 0, FVersionInfoSize,
      FVersionInfoBuffer);
    if FHaveVersionInfo then
    begin
      VerQueryValue(FVersionInfoBuffer, '\', FParam, FParameterLength);
      CopyMemory(@FFixedFileInfo, FParam, FParameterLength);
      VerQueryValue(FVersionInfoBuffer, '\VarFileInfo\Translation', FParam,
        FParameterLength);
      Temp := Integer(FParam^);
...
begin
  if VerQueryValue(FVersionInfoBuffer,
    PChar('\StringFileInfo\' + string(FLangChar) + '\' + ValueName), FParam,
