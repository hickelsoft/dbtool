...
type
  ...
  TGradientStyle = (gsHorizontal, gsVertical,
                    gsVerticalCenter, gsHorizontalCenter,
                    gsElliptic, gsRectangle);
...
begin
  ...
  BeginColor := clBtnFace; // Changed by HickelSOFT from clTeal to clBtnFace NOT FBeginColor!
  EndColor := clBtnFace; // Changed by HickelSOFT from clTeal to clBtnFace. NOT FEndColor!
...
var
  ...
  PanelRect : TRect;
begin
  if FInUpdate then exit; // added by HickelSOFT

  inherited;
  bmpGradient.Width := Width;
  bmpGradient.Height := Height;
  PanelRect := Rect(0,0,Width,Height);
  BeginR := FRealBeginColor and $000000ff;                   // Changed from FBeginColor to FRealBeginColor by HickelSOFT
  BeginG := (FRealBeginColor shr 8) and $000000ff;           // Changed from FBeginColor to FRealBeginColor by HickelSOFT
  BeginB := (FRealBeginColor shr 16) and $000000ff;          // Changed from FBeginColor to FRealBeginColor by HickelSOFT
  DiffR := (FRealEndColor and $000000ff) - BeginR;           // Changed from FEndColor to FRealEndColor by HickelSOFT
  DiffG := ((FRealEndColor shr 8) and $000000ff) - BeginG;   // Changed from FEndColor to FRealEndColor by HickelSOFT
  DiffB := ((FRealEndColor shr 16) and $000000ff) - BeginB;  // Changed from FEndColor to FRealEndColor by HickelSOFT
  ...
  PaintCaption;
  PaintBevel;
  BitBlt(Canvas.Handle,0,0,Width,Height,bmpGradient.Canvas.Handle,0,0,SRCCOPY);
end;

procedure THsGradientPanel.MouseDown(Button: TMouseButton;
                    Shift: TShiftState; X, Y: Integer);

begin
   if (Button = mbleft) and not Moving then Moving := True;
...
procedure THsGradientPanel.MouseUp(Button: TMouseButton;
              Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  if Alignment = taCenter then
    bmpGradient.Canvas.TextOut(((bmpGradient.Width - bmpGradient.Canvas.TextWidth(Caption)) div 2), ((bmpGradient.Height - bmpGradient.Canvas.TextHeight(Caption)) div 2), Caption);
  if Alignment = taLeftJustify then
    bmpGradient.Canvas.TextOut(OffSet,((bmpGradient.Height - bmpGradient.Canvas.TextHeight(Caption)) div 2),Caption);
  if Alignment = taRightJustify then
    bmpGradient.Canvas.TextOut((bmpGradient.Width - bmpGradient.Canvas.TextWidth(Caption) - OffSet), ((bmpGradient.Height - bmpGradient.Canvas.TextHeight(Caption)) div 2), Caption);
end;
...
begin
  ...
  if (BevelOuter <> bvNone) then begin
    ...
    for LineCount := 0 to (BevelWidth - 1) do begin
  ...
  if (BevelInner <> bvNone) then begin
    TopColor := clBtnHighlight;
    BottomColor := clBtnShadow;
    if (BevelInner = bvLowered) then begin
      TopColor := clBtnShadow;
      BottomColor := clBtnHighlight;
    end;
    if (BevelOuter <> bvNone) then begin
...
begin
  ...
  begin
    ...
    if FBeginColor < 0 then
      FRealBeginColor := GetSysColor(FBeginColor and $000000FF) // added by HickelSOFT
    else
...
begin
  ...
  begin
    FEndColor := Value;
    if FEndColor < 0 then
      FRealEndColor := GetSysColor(FEndColor and $000000FF) // added by HickelSOFT
    else
...
begin
  ...
  for I := 0 to 255 do begin
    ...
    B := bb + MulDiv(I, db, 255);
    bmpGradient.Canvas.Brush.Color := RGB(R, G, B);
    bmpGradient.Canvas.FillRect(Rect(Trunc(x1), Trunc(y1), Trunc(x2), Trunc(y2)));
  end;
  bmpGradient.Canvas.Pen.Style := psSolid;
end;
...
procedure THsGradientPanel.PaintHorizontalCenter(br, bg, bb, dr, dg, db : Integer);
var
  RectGradient : TRect;
  I : Integer;
  R, G, B : Byte;
