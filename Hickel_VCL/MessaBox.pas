...
uses
  ...
  ExtCtrls, Buttons, mmSystem, HsGauge, Menus, Clipbrd;
...
{$IFEND}
  ...
  published
    ...
    property BeforeExecute: TBeforeExecute read FBeforeExecute
...
begin
  ...
  begin
    ...
    Buttons := mbButtons;
    Style := mbStyle;
    if Assigned(AMoreInfoSL) then
      MoreInfoSL.Assign(AMoreInfoSL);
    result := Execute;
    Free;
  end;
end;
...
procedure TMessageBox.DoMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  ReleaseCapture;
...
begin
  ...
  try
    ...
    DLG_MoreInfoForm.DruckenSpeichern.Width := 130;
    DLG_MoreInfoForm.DruckenSpeichern.Caption := StrSchließen;
    DLG_MoreInfoForm.DruckenSpeichern.OnClick :=
...
begin
  if Assigned(DLG_MessageBox) then
    DLG_MessageBox.Close;
end;
